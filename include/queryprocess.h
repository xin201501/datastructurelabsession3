...
private:
  ...
  std::pair<std::string, std::string> splitLine(std::string_view line) {
    ...
    return std::make_pair(line.substr(lastSplitPoint + 1).data(),
                          std::string(line.data() + firstSplitPoint + 1,
                                      line.data() + lastSplitPoint));
  }
  std::string processLine(const Tree &tree, std::string_view line) {
    if (line.starts_with("whatis")) {
      return tree.printSubComponentInfo(
    ...
    if (line.starts_with("howmany")) {
      ...
      firstLetterUppercasedParentName[0] =
          std::toupper(firstLetterUppercasedParentName[0]);
      if (!parentInfo) {
        return "";
      }
      for (const auto &[hasComponentCount, subComponent] :
           (*parentInfo).get()->next) {
        if (subComponent->componentName == childName) {
          return firstLetterUppercasedParentName + " has " +
                 std::to_string(hasComponentCount) + ' ' + childName + '\n';
        }
      }
