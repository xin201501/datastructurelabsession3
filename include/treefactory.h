...
private:
	...
	std::tuple<std::string, size_t, std::string>
		...
		//转换数字结束
		//构造返回值
		return std::make_tuple(std::string(line.data(), line.data() + firstSplitPoint), hasComponentSize,
			std::string(line.data() + lastSplitPoint + 1));
	}

public:
	TreeFactory(std::string_view definitionFileName = defaultDefinitionFileName) :definitionFile(definitionFileName.data()) {
		//打不开文件抛异常
		if (!definitionFile) {
