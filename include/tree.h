...
struct TreeNode {
	...
	TreeNode(const TreeNode& another)
		...
		componentSize(another.componentSize), next(another.next) {}
	...
	std::string printSubComponentInfo() {
		std::string result("Part " + std::string(componentName) + " subparts are:\n");
...
private:
	...
	std::optional<std::reference_wrapper<std::shared_ptr<TreeNode>>>
		findComponentProcess(std::string_view componentName,
		...
		for (auto& [key, child] : beginSearchLocation->next) {
			result = findComponentProcess(componentName, child);
			if (result.has_value()) {
				return { *result };
			}
		}
	...
	void addComponent(std::string_view parentName, std::string_view componentName,
		...
		if (!postion) {
			if (head == nullptr) {
				...
				head->next.insert({ subComponentSize, std::make_shared<TreeNode>(componentName, subComponentSize) });
			}
			return;
		}
